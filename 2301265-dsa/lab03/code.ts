...
function parseExpression(
	...
	while (true) {
		let op = lexer.peek();
		if (!op || op.type === TokenType.EoF) break;
		const [leftBp, rightBp] = bindingPower(op.value);
		if (leftBp < minBp) break;
		lexer.next(); // consume operator
		let rhs = parseExpression(lexer, rightBp);
		if (!rhs) throw new Error("Invalid right-hand side");
		lhs = new Operation(op.value, lhs, rhs);
	}
	return lhs;
}
