...
#let qooklet(
  ...
  set page(
    ...
    footer: context {
      set text(size: 9pt, font: fonts.at(lang).title)
  ...
  set heading(
    numbering: (..numbers) => if numbers.pos().len() == 1 {
      return numbering("บทที่ 1", ..numbers)
    } else {
  ...
  show raw: set text(font: ("RobotoMono Nerd Font", "Noto Sans Thai Looped", "SF Thonburi"), size: 10pt)
...
// text
#let fonts = toml("fonts.toml")
#let ctext(body) = text(body, font: fonts.at("th").math)
