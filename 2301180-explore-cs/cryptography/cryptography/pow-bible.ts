async function readBibleText() {
  return (await Bun.file("./bible.txt").text()).split("\n").map(line => line.trim()).filter(line => line.length > 0);
}

const bibleText = await readBibleText()
...
async function proofOfWork(
  ...
  do {
    input = `${text}+${bibleText[nonce % bibleText.length]}`;
    hash = await hashText(input);
    console.log(`Trying nonce: ${nonce}, Hash: ${hash}`);
    nonce++;
...
console.log(`Nonce text: ${bibleText[hash.nonce % bibleText.length]}`);
